...
from . import torch_utils2, google_utils2  #  torch_utils, google_utils
...
def xyxy2xywh_bot(x):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right
    y = torch.zeros_like(x) if isinstance(x, torch.Tensor) else np.zeros_like(x)
    centerX = (x[:, 0] + x[:, 2]) / 2  # x center
    centerY = (x[:, 1] + x[:, 3]) / 2  # y center
    y[:, 2] = x[:, 2] - x[:, 0]  # width
    y[:, 3] = x[:, 3] - x[:, 1]  # height
...
def non_max_suppression2(prediction, conf_thres=0.1, iou_thres=0.6, fast=False, classes=None, agnostic=False):
...
def distancing_bot(people_coords, person_points, img, dist_thres_lim=(200,250)):
    # Plot lines connecting people
    already_red = dict() # dictionary to store if a plotted rectangle has already been labelled as high risk
    centers = []
    for i in people_coords:
    ...
    #x_combs = list(itertools.combinations(person_points,2))
    arr_combs = list(itertools.combinations(arr,2))
    radius = 10
    thickness = 5
    count = 0
    #for x in x_combs:
    for a in arr_combs:
        xyxy1_dist, xyxy2_dist = person_points[a[0]],person_points[a[1]]
        xyxy1, xyxy2 = people_coords[a[0]],people_coords[a[1]]
        #cntr1 = ((int(xyxy1[2])+int(xyxy1[0]))//2,(int(xyxy1[3])+int(xyxy1[1]))//2)
        #cntr2 = ((int(xyxy2[2])+int(xyxy2[0]))//2,(int(xyxy2[3])+int(xyxy2[1]))//2)
        #dist = ((cntr2[0]-cntr1[0])**2 + (cntr2[1]-cntr1[1])**2)**0.5
        
        cntr1_d = int(xyxy1_dist[0]),int(xyxy1_dist[1])
        cntr2_d = int(xyxy2_dist[0]),int(xyxy2_dist[1])
        dist = ((cntr2_d[0]-cntr1_d[0])**2 + (cntr2_d[1]-cntr1_d[1])**2)**0.5
        print(dist) 
        cntr1 = int(xyxy1[0]),(int(xyxy1[1])+(int(xyxy1[3])//2))
        cntr2 = int(xyxy2[0]),(int(xyxy2[1])+(int(xyxy2[3])//2))

        if dist > dist_thres_lim[0] and dist < dist_thres_lim[1]:
            count = count + 1
            color = (0, 255, 255)
            label = "Low Risk "
            print("Low Risk:"+str(dist))
            cv2.line(img, cntr1, cntr2, color, thickness)
            #if already_red[cntr1] == 0:
            cv2.circle(img, cntr1, radius, color, -1)
            #if already_red[cntr2] == 0:
            ...
            for ab in a:
                xy = people_coords[ab]
                #cntr = ((int(xy[2])+int(xy[0]))//2,(int(xy[3])+int(xy[1]))//2)
                cntr = int(xy[0]),int(xy[3])//2+int(xy[1])
                #if already_red[cntr] == 0:
                c1, c2 = (int(xy[0]), int(xy[1])), (int(xy[2]), int(xy[3]))
                tf = max(tl - 1, 1)  # font thickness
                t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
